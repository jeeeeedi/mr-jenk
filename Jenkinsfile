...
SSHEOF
    ...
    post {
        success {
            script {
                try {
                    withCredentials([string(credentialsId: 'team-email', variable: 'TEAM_EMAIL')]) {
                        echo '=========================================='
                    ...
                    echo "ℹ️  Ensure 'team-email' credential is configured in Jenkins"
                }
            }
        }
        failure {
        ...
        unstable {
            script {
                try {
                    ...
                    echo "⚠️  Could not send unstable email: ${e.message}"
                    echo "ℹ️  Ensure 'team-email' credential is configured in Jenkins"
                }
            }
        }
